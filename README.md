...
- [Seismic Foundry](https://docs.seismic.systems/getting-started/installation) (`sforge` CLI)
...
cp .env.example .env
# Edit .env with your Fireblocks + Seismic credentials

# 3. Install Solidity dependencies
cd contracts
sforge soldeer install

# 4. Deploy contracts + run single-token demo
cd ..
bun run src20:deploy
bun run src20:demo

# 5. Deploy multi-token contracts + run batch demo
...
Contracts depend on [seismic-std-lib](https://github.com/SeismicSystems/seismic-std-lib) via Soldeer. This must be run before building or deploying:
...
Each demo has a deploy step followed by a run step. The deploy scripts output contract addresses to `contracts/out/` which the demos read at runtime.

**Single-token demo:**

```bash
bun run src20:deploy           # Deploys TestSRC20 → contracts/out/deploy.json
bun run src20:demo             # Runs the 9-step demo
```

**Multi-token demo:**

```bash
bun run deploy:multi-token     # Deploys 50 SRC20 tokens + SRC20Multicall → contracts/out/batch-read-deploy.json
bun run src20:demo-multi-token # Runs the batch read benchmark
```
...
contracts/
├── src/
│   └── TestSRC20.sol              # Admin-restricted SRC20 token (extends seismic-std-lib/SRC20)
├── utils/
│   └── MockSRC20.sol              # Public mint/burn SRC20 for testing
├── script/
...
├── dependencies/                  # Soldeer deps (gitignored, run sforge soldeer install)
│   └── seismic-std-lib-0.1.1/    # SRC20, SRC20Multicall, ISRC20, etc.
└── out/
    ├── deploy.json                # Single-token addresses (generated by deploy.sh)
    └── batch-read-deploy.json     # Multi-token + multicall addresses (generated by deploy-batched-read.sh)
```
